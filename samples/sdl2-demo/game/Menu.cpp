...
void Menu::update(const InputManager& input, float delta)
{
	...
	if (yAxis.is_set() && !yAxis.was_set()) {
		if (yAxis.value < 0) {
			if (m_current_item == 0) {
				m_current_item = static_cast<unsigned short>(m_menu_items.size());
			}
			--m_current_item;
		} else {
			++m_current_item;
		}
		if (m_current_item >= m_menu_items.size()) m_current_item = 0;
	}
...
void Menu::render()
{
	...
	for (auto& m_menu_item : m_menu_items) {
		m_renderer.draw_text( m_font, pos, m_menu_item->label(), Font::ALIGN_CENTER,
							  i == m_current_item ? active : inactive );
		pos.y += m_font->height();
		++i;
	}
}
